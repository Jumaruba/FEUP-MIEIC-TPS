...
Result nearestPoints_BF(vector<Point> &vp) {
	...
	for (int i = 0; i < vp.size(); i++){
	    for (int j = i+1; j < vp.size(); j++){
	        if (j == i) continue;
	        if (vp[j].distance(vp[i]) < res.dmin) res = Result(vp[j].distance(vp[i]), vp[i], vp[j]);
	    }
	}
	return res;
}
...
Result nearestPoints_BF_SortByX(vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); i++){
        ...
        for (int j = i+1; j < vp.size(); j++){
            if (j == i) continue;
            if (vp[i].distance(vp[j]) < res.dmin) res = Result(vp[i].distance(vp[j]), vp[j], vp[i]);
            else if (vp[i].distance(vp[j]) > res.dmin && vp[j].x - vp[i].x >= dist ) break;
            else if (vp[i].distance(vp[j]) > res.dmin && dist == MAX_DOUBLE) dist = vp[i].distance(vp[j]);
        }
    }
	return res;
}
...
static void npByY(vector<Point> &vp, int left, int right, Result &res)
{
    ...
    double lambda = vp[left].distance(vp[right]);
    for (int i = left; i < right+1; i++ ){
        for (int j = i+1; j < left+1; j++){
            if (vp[i].distance(vp[j]) < lambda) res = Result(vp[i].distance(vp[j]), vp[i], vp[j]);
            else if (vp[i].y - vp[j].y > lambda) break;
        }
    }
}
