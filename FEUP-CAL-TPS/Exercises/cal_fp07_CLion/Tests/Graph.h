...
public:
	...
	friend bool operator<(Edge<T> e1, Edge<T> e2){
...
bool Graph<T>::addBidirectionalEdge(const T &sourc, const T &dest, double w) {
    if (!addEdge(sourc,dest,w)) return false;
    return addEdge(dest, sourc, w);
}


//the option of not using the previous functions was for study purpose
template <class T>
vector<Vertex<T>* > Graph<T>::calculatePrim() {
	...
	this->vertexSet[0]->dist = 0;
	MutablePriorityQueue<Vertex<T>> q;
	q.insert(this->vertexSet[0]);
	while(!q.empty()){
	    Vertex<T> *v = q.extractMin();
	    vertexSet.push_back(v);                     //add the path
	    v->visited = true;
	    for (Edge<T> e: v->adj){
...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
    ...
    int accepted = 0;
	DisjointSet d(this->vertexSet.size());
    priority_queue<Edge<T>> q;


    //remember that each vertex is being added twice
    for (Vertex<T>* v: this->vertexSet){
        v->path = nullptr;
        for (Edge<T> e: v->adj){
            q.push(e);
    ...
    result.push_back(vertexSet[0]);
    while(accepted < vertexSet.size()){
        Edge<T> e = q.top();
        q.pop();
        int orig = d.find(e.orig->getInfo());
        int dest = d.find(e.dest->getInfo());
        if (orig != dest){            //get the set
            d.unionSets(e.orig->getInfo(), e.dest->getInfo());
            accepted++;
            e.dest->path = e.orig;
        }
