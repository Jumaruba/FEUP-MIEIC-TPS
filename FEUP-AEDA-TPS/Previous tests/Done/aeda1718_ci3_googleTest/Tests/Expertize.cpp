...
bool Expertize::operator<(const Expertize &exp1) const {
    if (this->name < exp1.name) return true;
    else if(this->name > exp1.name) return false;
    else if (this->cost < exp1.cost) return true;
    else if (this->cost > exp1.cost) return false;
    return false;
}
