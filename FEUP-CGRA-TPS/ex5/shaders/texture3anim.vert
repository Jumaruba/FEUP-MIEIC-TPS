...
void main() {
	...
	if (texture2D(uSampler2, vec2(0.0,0.1)+vTextureCoord).b > 0.5)
		offset=aVertexNormal*normScale*0.1*sin(timeFactor);				//moving just the normal vertex

	offset2 = vec3(1.0,0.0,0.0)* normScale * 0.1 * sin(timeFactor);		//moving in the x-axis
